...
export async function callGemini(promptText: string) {
  try {
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${process.env.NEXT_PUBLIC_GEMINI_API_KEYNEXT_PUBLIC_GEMINI_API_KEY}`,
      {
        method: "POST",
